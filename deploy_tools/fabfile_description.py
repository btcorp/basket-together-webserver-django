...
## repo/PROJECT_NAME만 변경하면 됨.
env.use_ssh_config = True
REPO_URL = 'https://github.com/btcorp/basket-together-apiserver-django.git'   # git repo url setting
...
def deploy():
    ...
    _chown_database(source_folder)


# 프로젝트 구조에 맞춰 디렉토리 생성
def _create_directory_structure_if_necessary(site_folder):
    # 'database', app, 'static', 'virtualenv', 'media' 디렉토리 생성.
...
def _chown_database(source_folder):
    print('==========================chown database directory & file============================================')
    run('cd {0}/../database/ && sudo chown www-data:www-data . db.sqlite3'.format(source_folder))